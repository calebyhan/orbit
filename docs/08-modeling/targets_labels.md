...
* **ETF return:** $r^{ETF}\_{t+1} = \frac{P\_{t+1}}{P\_t} - 1$
* **Index return:** $r^{SPX}\_{t+1} = \frac{I\_{t+1}}{I\_t} - 1$ where `I_t` is ^SPX close.
...
* `next_open`: $P\_t = \mathrm{Close}\_t$, $P\_{t+1} = \mathrm{Open}\_{t+1}$
* `next_close`: $P\_t = \mathrm{Close}\_t$, $P\_{t+1} = \mathrm{Close}\_{t+1}$
...
* **Direction (ETF):** $y\_{t}^{clf} = \mathbb{1}\\{r^{ETF}\_{t+1} > 0\\}$
* **Direction (Excess):** if `labels.use_excess: true`, then $y\_{t}^{clf} = \mathbb{1}\\{r^{EX}\_{t+1} > 0\\}$

### 2) Regression

* **Raw return (ETF):** $y\_{t}^{reg} = r^{ETF}\_{t+1}$
* **Excess return:** if `labels.use_excess: true`, use $y\_{t}^{reg} = r^{EX}\_{t+1}$
